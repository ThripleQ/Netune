...
- 导航栈：`Esc` 返回上一层
- 内置配置与维护工具 `netune --manage`（主题编辑 / 按键绑定 / 本地音乐目录 / 音频缓存）
...
> `--config` 保留为兼容别名，两者进入同一界面。

六个页面：主题（`x` 进颜色槽编辑，`Enter` 选用）、按键（编辑/选用）、本地音乐（`a` 添加目录）、播放（音量/循环/步长）、缓存（`d` 清空音频缓存）。也可以直接改 `config.json`：
